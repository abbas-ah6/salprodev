...
const FaqCard: React.FC<{
  ...
  >
    <div className="flex w-full justify-between items-center pb-[12px]">
      <span
        className={`m-0 lg:text-b18 text-b16 text-brand-black font-normal w-full max-w-full`}
      >
        {faq?.question || ""}
      </span>
      <Image
...
const Faqs: React.FC<IFaqs> = ({ description, heading, image, faqs }) => {
  ...
  const faqsStructuredData: FAQPage | undefined =
    faqs && faqs.length > 0
      ? {
          "@type": "FAQPage",
      ...
      {faqsStructuredData && (
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(faqsStructuredData),
          }}
        />
      )}
